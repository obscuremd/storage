...
const gradientMapping: Record<string, string> = {
  ...
  grey: "linear-gradient(hsl(0, 0%, 60%), hsl(0, 0%, 40%))"
};

const GlassIcons: React.FC<GlassIconsProps> = ({ label, color, icon, onclick }) => {
  ...
  return (
        ...
        <button
          ...
          onClick={onclick}
          aria-label={label}
          className={`relative cursor-pointer bg-transparent outline-none w-[4.5em] h-[4.5em] [perspective:24em] [transform-style:preserve-3d] [-webkit-tap-highlight-color:transparent] group`}
        >
          <span
